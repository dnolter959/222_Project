...
fdf %>% 
 ...
 group_by(Model) %>% dplyr::summarise(across(c("mse"), median)) %>% kable(caption = "Median Mean Squared Error values under various estimation procedures")

```
The mean squared errors for predictions of elastic net, ridge, and LASSO regression are all smaller than the mean squared error for unpenalized regression. This means that the bias-variance trade-off is working for LASSO and Ridge regression in this situation. 
