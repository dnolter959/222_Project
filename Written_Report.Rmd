...
• Ridge regression will include all p predictors in the final model - this can be a problem
for high dimensional and some other datasets!
• The penalty in Ridge regression will shrink all of the coefficients towards zero, but it
will not set any of them completely to zero, as we found above.
• This may not be a problem for prediction accuracy, but it can create challenges in
model interpretation and parsimony!
...
We begin with a basic We would like to predict Y given a set of 10 covariates using the linear model below. 
...
```
Where we set alpha equal to 0, 0.5, and 1 for Ridge, Elastic Net, and LASSO, respectively. For selecting the lambda level, we first ran each model with no lambda parameter and used 5-fold cross validation to select the lambda level associated with the lowest MSE in each case. Then we fit each penalization model with this lambda value.
...
Here are a series of boxplots which show the distribution of point estimates for each of the 11 $\beta$ parameters. Recall that for the simulation study we set $\beta_0 = 2$, $\beta_1 = 1$, $\beta_2 = 0.5$, $\beta_3 = -0.5$, and $\beta_4 = \beta_5 = ... = \beta_9 = \beta_{10} = 0$. In this setting we set n = 300, and run each model 500 times. 
...
We've now reviewed the estimates of the *beta coefficients* relative to their true underlying values. A similar but related question we'd like to answer is how well each model performs at predicting the true underlying *Y values*. In order to do this, we compare models using **out-of-sample MSE** (described above). 
...
df3 = df2 %>% filter(num_obs == 200, sparsity == 0.5, Small == 0)
ggplot(df3, mapping = aes(col = Model, x = 0, y = log(MSE))) + 
  geom_boxplot() +
  theme(axis.title.x=element_blank(),
      axis.text.x=element_blank(),
      axis.ticks.x=element_blank())
```
It is clear that in this setting of relatively high sparsity Elastic Net and LASSO outperform Ridge and un-penalized OLS in terms of out-of-sample MSE. 
...
As stated in the simulation design portion above, in order to fit our penalization models, we simply used cross validation to select the lambda value associated with the smallest MSE, and then used this lambda value in our final regression fit. A related consideration is to examine results for different levels of `lambda` in LASSO regression, and observe sensitivity of results for this varying levels of lambda.
...
As you can see, as the value of lambda increases, as does the penalty applied to the beta coefficients, and for a sufficiently large value of lambda (in this case, about 1.25), all $\beta$ coefficients are sent to 0. Our modeling process selects the level of lamda using 5-fold cross validation such that the out-of-sample MSE (i.e., the MSE in the test sets of the cross validation) is minimized; ideally, this level of lambda will be one where all truly 0 $\beta$ coefficients are sent to 0 and non-zero ones are not.   
