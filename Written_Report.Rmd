...
First, we will evaluate the prediction MSE for each model under different levels of sparsity (.1, .5, and .98), and at different sample sizes (20, 100, 500). A sparsity of 0.1 means that 10% of the true beta coefficients are 0. We will use 500 replications under each condition to conduct estimation, and we will use 100 covariates in each simulation. 
...
Here we see that OLS performs about as well as Elastic Net and LASSO in the low sparsity, low smallness setting, and OLS has especially poor performance, relatively speaking, in the high smallness setting. In each case, higher sample sizes lead to convergence of MSE across the fur models.
