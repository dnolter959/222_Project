...
Now we consider the setting where the non-zero coefficients are close to 0, (i.e., "small"). In this setting, we fix sparsity at 0.1 (i.e., 2 of the 20 parameters are truly 0) and vary the size of non-zero coefficients. We set small coefficients to lie in the range 0.0005-0.005, an we allow the remaining coefficients to lie in the range 0.5-1.5. So for example, in the setting with `small = 0.5`; half of the true non-zero coefficients lie in 0.0005-0.005, and half lie in 0.5-1.5. In this setting, we expect that severe penalization methods like LASSO and Elastic Net will send too many of these coefficients to 0, and the OLS model will overestimate these small true $\beta$ values. 
...
Indeed, we see that Ridge regression is the highest performing model in the setting of high "smallness", and this is especially true with a lower number of observations. However, Elastic Net and LASSO perform quite well in this contrived example as well. This is because these models are estmimating the small coefficients to be 0; and since 0 is very close to their true values, the predicted Y values are close to their true values. To expand this study and more clearly demonstrate the superiority of Ridge in this setting, we might slightly increasing the size of true small parameters values, or increasing the number of covariates in each model. 
