...
**Mean Square Error (MSE) and Variance of Regression Coefficient**

We calculated the MSE of regession coefficient for each beta coefficients. To calculate this, for each model ran during the simulation, we subtracted estimated betas fromm each respetive true beta parameters. We used the "median" MSE and plotted the beta difference values across all 500 simnulations as box plots. A "median" MSE was used because it is more robust to outliers (extreme observations that occur from chance). The variance of each beta across the 500 simulations were also determined and plotted.
...
First, we will evaluate the prediction MSE for each model under different levels of sparsity (.1, .5, and .98), and at different sample sizes (200, 800, 1600). A sparsity of 0.1 means that 10% of the true beta coefficients are 0. We will use 500 replications under each condition to conduct estimation, and we will use 100 covariates in each simulation. 
...
In conclusion, through our study we found that:

Main findings:
- For non-zero beta coefficients, LASSO, Ridge, and Elastic Net underestimate (in absolute value terms) the true paramter while the unpenalized model is unbiased
  0 using penalization methods such as LASSO, Ridge, and Elastic Net, while powerful, introduce biased when predicting relatively "large" paramter values and hsould be used with care

- For beta coefficients that are zero, the penalization methods (LASSO, Ridge, and Elastic Net) outperformed the unpenalized OLS in that they haev nuch lower variance and more accurately predict the true 0 betas

- For true 0 betas, Elastic Net and LASSO regression have lower MSE for coefficients as compared with unpenalized OLS
- For true 0 betas, the variance in beta estimates is much lower for Elastinet Net and LASSO but is comparable to unpenalized OLS for non-zero coefficients.

- In this setting of relatively high sparsity, Elastic Net and LASSO outperform Ridge and un-penalized OLS in terms of out-of-sample MSE
   This is because bias introduced by shrinking the non-zero parameters in the penalized models is only a small price to pay for the steep reduction in variance when predicting the truly 0 parameter values. In this setting where there is high sparsity, it is clear that LASSO reigns supreme.

- In high sparsity settings (spasity at 0.98), the unpenalized OLS model performs poorly relative to the penalization methods especially when n is small. 

- When non-zero coefficients are close to 0, (i.e., “small”), at a fixed sparsity of 0.1, and vary the size of non-zero coefficients. We set small coefficients to lie in the range 0.01-0.1, an we allow the remaining coefficients to lie in the range 0.5-1.5, we find that Ridge regression is the highest perofming model in the setting of high smallness especialyl with lower observations. As sparsity increases, LASSO and Elastic Net perofmr as well or better than Ridge
